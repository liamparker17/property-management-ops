...
| format.ts | `FINANCIAL_YEAR_START`, `formatZar(cents)`, `formatZarShort(cents)` (abbreviated `R 5k` / `R 1.2M` / `R 92M` for narrow tile values), `formatDate(d)` | 22 |
...
| /dashboard | (staff)/dashboard/page.tsx | Staff Overview: 7-KPI hero band with sparklines (responsive font scaling, abbreviated currency), Invoiced-vs-Collected combo chart with prior-period overlay, status strip, **480px Map hero band** with `?view=map\|table` toggle (Phase 3) — pins colored by composite health, click opens property-detail drill, table view = `PropertyHealthRanking`; 4-up cockpit grid (arrears aging / occupancy donut / lease expiries / maintenance spend), 3-up (top-10 overdue table / urgent maintenance list / utility recovery), open-maintenance ranked list, maintenance-by-status; Phase 2b: 4 drillable tiles + property pins carry "View detail" links setting `?drill=<tileId>`; threads AnalyticsCtx from URL search params; renders DrillSheet overlay when `?drill=` set (5 drill ids: arrears-aging, top-overdue, lease-expiries, urgent-maintenance, property-detail) |
...
| lib/zod/analytics-drill.ts | `drillIdSchema`, `DrillId` — Zod enum of drill ids (arrears-aging / top-overdue / lease-expiries / urgent-maintenance / property-detail; Phase 3 adds `property-detail`) | 14 |
| lib/analytics/health-band.ts | `HealthBand`, `healthBandColor(score)`, `HEALTH_BAND_HEX` — Leaflet-free composite health band → color helper used by map pins, ranking table, drill renderers | 18 |
...
| components/analytics/maps/portfolio-pins.tsx | `PortfolioPin`, `PortfolioPins` - React-Leaflet portfolio marker renderer with custom `divIcon` pins; Phase 3: pins colored by `healthScore` band, Popup renders an Open-detail link via `hrefBuilder` (used by Overview to wire `?drill=property-detail&propertyId=...`); re-exports `healthBandColor` + `HealthBand` from `lib/analytics/health-band.ts` for back-compat | 95 |
| components/analytics/maps/map-panel.tsx | `MapPanel` — wraps `PortfolioPins` with a card chrome; Phase 3 forwards `hrefBuilder` prop down for pin-click navigation | 50 |
| components/analytics/map-table-toggle.tsx | `MapTableToggle` — Phase 3 client component flipping `?view=map\|table` for the Overview map hero band | 50 |
| components/analytics/property-health-ranking.tsx | `PropertyHealthRanking` — Phase 3 sortable table over `PortfolioView.rows` with health badge column (default sort: healthScore desc); used as the table view of the Overview map hero band | 70 |
| components/analytics/drill/property-detail-drill.tsx | `PropertyDetailDrill` — Phase 3 drill renderer for pin-click side panel: 4 KPI cards + recent-expiries table + recent-maintenance table + Open-property link | 130 |
