...
export default async function LandlordReportDetailPage({ params }: Props) {
  ...
  if (!user?.landlordId) redirect('/landlord/reports');
  ...
  if (!pack) redirect('/landlord/reports');
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Accountant-ready PDF and CSV support files for your portfolio."
        actions={
          <>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <MetricCard label="Income" value={formatZar(totals.incomeCents ?? 0)} />
        <MetricCard label="Expenses" value={formatZar(totals.expenseCents ?? 0)} />
