...
export default async function LandlordStatementsPage({ params }: Props) {
...
async function Content({ id }: { id: string }) {
  ...
  const statements = await listStatements(ctx, { type: 'LANDLORD', subjectType: 'Landlord', subjectId: id });

  return (
    <div className="space-y-6">
      <div>
        <Link href={`/trust/landlords/${id}`} className="inline-flex items-center gap-1.5 text-xs font-medium uppercase tracking-wider text-muted-foreground hover:text-foreground">
          <ArrowLeft className="h-3.5 w-3.5" />
          Back to landlord trust
        </Link>
      </div>
      <PageHeader
        eyebrow="Landlord statements"
        title={landlord.name}
        description={`${statements.length} statement${statements.length === 1 ? '' : 's'} on file.`}
      />

      <Card>
        <CardHeader><CardTitle className="text-base">Generate landlord statement</CardTitle></CardHeader>
        <CardContent>
          <GenerateStatementForm endpoint={`/api/statements/landlords/${id}`} />
        </CardContent>
      </Card>

      {statements.length === 0 ? (
        <EmptyState title="No landlord statements yet" description="Generate one for the period you want to share with the landlord." />
      ) : (
